...
class ModelExtensionPaymentGOPCOD extends Model
{
	...
	{
		if($this->config->get($this->extension_name . $this->split_char . 'status'))
		{
			...
			$shipping_method_status = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, "status"]));

			if($shipping_method_status == 0)
			{
				$shipping_method_status = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, "status"]));
			}
			else
			{
			...
			{
				$shipping_geo_zone = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, "shipping_geo_zone"]));

				if($shipping_geo_zone == 0)
				{
					$shipping_geo_zone = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, "shipping_geo_zone"]));
				}
				else
				{
				...
				if($shipping_geo_zone == 1)
				{
					...
					if(isset($this->session->data['shipping_method']['code']))
					{
						$shipping_method_geo_zone = explode($this->split_char, $this->session->data['shipping_method']['code']);
				...
				if($shipping_method_geo_zone_id == 0)
				{
					...
					if(count($zone_to_geo_zone_results) > 1)
					{
						...
						{
							$shipping_method_geo_zone_status = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $zone_to_geo_zone_result['geo_zone_id'], $customer_group_id, "status"]));

							if($shipping_method_geo_zone_status == 0)
							{
								$shipping_method_geo_zone_status = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $zone_to_geo_zone_result['geo_zone_id'], $customer_group_id, "status"]));
							}
							else
							{
				...
				$shipping_method_geo_zone_status = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "status"]));

				if($shipping_method_geo_zone_status == 0)
				{
					$shipping_method_geo_zone_status = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "status"]));
				}
				else
				{
					$shipping_method_geo_zone_status = $shipping_method_geo_zone_status - 1;
				...
				{
					//Shipping Sort Order
					$shipping_method_sort_order = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, "sort_order"]));

					if($shipping_method_sort_order == "")
					{
						$shipping_method_sort_order = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, "sort_order"]));
					}
					...
					$shipping_method_tax_class_id = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "tax_class_id"]));

					if($shipping_method_tax_class_id == 0)
					{
						$shipping_method_tax_class_id = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "tax_class_id"]));
					}
					...
					$shipping_method_order_status_id = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "order_status_id"]));

					if($shipping_method_order_status_id == 0)
					{
						$shipping_method_order_status_id = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "order_status_id"]));
					}

					//Shipping Geo Zone Method
					$shipping_method_method = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "method"]));

					if($shipping_method_method == 0)
					{
						$shipping_method_method = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "method"]));
					}
					else
					{
					...
					if($shipping_method_method == 0)
					{
						$shipping_method_flat = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "flat"]));

						if($shipping_method_flat == "")
						{
							$shipping_method_flat = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "flat"]));
						}
						...
						$shipping_method_flat_currency = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "flat_currency"]));

						if($shipping_method_flat_currency == 0)
						{
							$shipping_method_flat_currency = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "flat_currency"]));
						}
					...
					{
						$shipping_method_percent = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "percent"]));

						if($shipping_method_percent == "")
						{
							$shipping_method_percent = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "percent"]));
						}
						...
						$cost = ($this->tax->calculate($this->getTotal(), $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "tax_class_id"])), $this->config->get('config_tax'))) * ($shipping_method_percent / 100);
					}
					else
					{
						$shipping_method_custom = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "custom"]));

						if($shipping_method_custom == "")
						{
							$shipping_method_custom = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "custom"]));
						}
					...
					//Enable Rule
					$enable_rule = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "enable_rule"]));

					if($enable_rule == "")
					{
						$enable_rule = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "enable_rule"]));
					}
					...
					//Order Total
					$shipping_method_order_total = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "order_total"]));

					if($shipping_method_order_total == 0)
					{
						$shipping_method_order_total = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "order_total"]));
					}
					else
					{
					...
					$shipping_method_order_total_sort_order = $this->config->get(implode($this->split_char, [$this->extension_name, $shipping_method, $shipping_method_geo_zone_id, $customer_group_id, "order_total_sort_order"]));

					if($shipping_method_order_total_sort_order == "")
					{
						$shipping_method_order_total_sort_order = $this->config->get(implode($this->split_char, [$this->extension_name, $this->extension_default, $shipping_method_geo_zone_id, $customer_group_id, "order_total_sort_order"]));
					}
